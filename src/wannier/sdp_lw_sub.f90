...
!                                                   
! 
subroutine steepest_descent_lw_sub
      use OMP_LIB
      use commvar
      implicit none
      ...
      if((OBJ_1-OBJ_sv).gt.sub_thre) exit
 ...
 if((OBJ_1-OBJ_sv).gt.sub_thre) then
   write(6,*) '---------------------------'
   write(6,*) 'subspace wannierization'; call flush(6)
   write(6,*) '---------------------------'
   nocc_sub = nocc_LW * sub_fac 
   max_iter = max_iter - count_iter
   OBJ_0 = 0d0
   OBJ_1 = 0d0
   count_iter = 0
  
   do i=1,n_occ
      orb_indx_sub = maxloc(orb_con_sv,n_occ)
      orb_con_sv(orb_indx_sub) = 0d0
      CO(:,i) = LO(:,orb_indx_sub)
   end do
...
end subroutine steepest_descent_lw_sub
